 ...
 */
template <const uint8_t BOOT_SUBCLASS, const uint8_t BOOT_PROTOCOL>
HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::HIDBoot(USBHost *p) :
		HID(p),
		pRptParser(NULL),
		qNextPollTime(0),
		bPollEnable(false)
 ...
 */
template <const uint8_t BOOT_SUBCLASS, const uint8_t BOOT_PROTOCOL>
void HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::Initialize()
{
 ...
 */
template <const uint8_t BOOT_SUBCLASS, const uint8_t BOOT_PROTOCOL>
uint32_t HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::Init(uint32_t parent, uint32_t port, uint32_t lowspeed)
{
 ...
 */
template <const uint8_t BOOT_SUBCLASS, const uint8_t BOOT_PROTOCOL>
void HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::EndpointXtract(uint32_t conf, uint32_t iface, uint32_t alt, uint32_t proto, const USB_ENDPOINT_DESCRIPTOR *pep)
{
 ...
 */
template <const uint8_t BOOT_SUBCLASS, const uint8_t BOOT_PROTOCOL>
uint32_t HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::Release()
{
...
uint32_t HIDBoot<BOOT_SUBCLASS, BOOT_PROTOCOL>::Poll()
{
	...
	if (qNextPollTime <= millis())
	{
		...
		const uint32_t const_buff_len = 64;
